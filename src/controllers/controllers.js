...
const updateBlogs = async (req, res) => {
    try {
        ...
        let updateBlogs = await blogsModel.findOneAndUpdate({ _id: blogId }, { $addToSet: { tags: tags, subcategory: subcategory }, 
            title: title, body: body, category: category,isPublished:isPublished},{ new: true })
...
/*let deletByQuery = async (req, res) => {
    ...
    try {
        ...
        let query = {
            isDeleted: false,
            authorId:req.headers["decoded-token"] 
        }
        if (data.tags) {
        ...
        const result = await blogsModel.updateMany(query, { $set: { isDeleted: true, DeletedAt: new Date().toLocaleString() } })
...
const deletByQuery = async (req,res) => {
    try{
        ...
        let id =req.headers["decoded-token"] 
        let findBlogs = (await blogsModel.find({$and: [req.query,{authorId: id},{isDeleted: false},{isPublished: false}]}))
        console.log(findBlogs);
        if(!findBlogs.length) 
            return res.status(404).send({status: false, msg: "No documents found."})
        /***************************************************************************************/
        let blogs = await blogsModel.updateMany({_id: findBlogs},{isDeleted:true, deletedAt: Date.now()})
        ...
        //let deletData = await blogsModel.find(data1).select({authorId:1,_id:0}) 
                  //finding documnet form blogsCollection using blogsModel
        //blogs.push(deletData);
     

       /*let blogs=function (){
       if( req.headers["decoded-token"]===deletData[0].authorId)  return blogs;
       }*/
...
//console.log(req.headers["decoded-token"]);
//blogs.map((blog)=>{
  //      if(  blog.authorId==req.headers["decoded-token"]) {
    //        console.log("yes");
      //  }
    //})
        ...
        //const findData=await blogsModel.find(deletData[0]._id).select({_id:1})
        /*const findData1=await blogsModel.updateMany(
            {findData},
            {$set:{isDeleted:true}},
             {deletedAt:new Date()}
            )*/
